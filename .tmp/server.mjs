...
async function main() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    logger_default.info(`\u{1F680} Kiddies Town Academy server running on http://localhost:${PORT}`);
    logger_default.info(`\u{1F4CB} Environment: ${process.env.NODE_ENV || "development"}`);
  });
}
main().catch((err) => {
  logger_default.error({ error: err, message: err.message, stack: err.stack, code: err.code }, "\u{1F4A5} Failed to start server");
  process.exit(1);
});
